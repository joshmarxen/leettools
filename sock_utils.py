...
def wait_for_text(con, t, to=0.25, times_to_try=5):
    l = clear_read_buffer(con,to)
    n = 0
    while not(t(l)) and n < times_to_try:
        l += clear_read_buffer(con, to)
        n += 1
    return l
...
def go_interactive(con):
    ...
    while True:
        print "remote shell >",
        comm = stdin.readline().replace("\n", "")
        print "command: "+comm
        if comm == "exit":
            return
        con.send_line(comm)
        print clear_read_buffer(con,1)